...
const ENEMY_TYPES = {
  ...
  titan:       { spr: 'titan',       label: 'Titã Carrasco',      hp: 1300, w: 104, h: 188, speed: 64,  aggro: 1000, range: 620, fireCd: 1.5, touch: 34, score: 3400, boss: true, mythos: true, name: 'O CARRASCO INFERNAL',       gore: '#7a1a14', meleeVisual: 'greataxe', meleeBlade: '#9aa2ad' },
};
...
class Enemy extends Entity {
  ...
  hurt(dmg, dir, game) {
    ...
    if (this.freezeT > 0) { dmg = Math.round(dmg * 1.6); game.fx.iceShatter(this.cx, this.cy, 12); this.freezeT = 0; this._shatter = 0.3; }  // estilhaça o gelo (dano extra)
  ...
  // MORTES VARIADAS pela causa do golpe fatal:
  //  · congelado  → o corpo ESTILHAÇA em cacos de gelo (sem cadáver)
  //  · em chamas  → tomba carbonizando (cadáver chamuscado + brasas/fumaça)
  //  · overkill   → explode em pedaços (gore total, sem cadáver)
  //  · normal     → tomba e o CADÁVER FICA no chão (persistente)
  die(game, dir = 0) {
    if (this.dying != null) return;
    const gore = this.def.gore, big = this.boss ? 3 : this.mini ? 2 : 1;
    const overkill = this.hp < -this.maxhp * 0.5;
    const cause = (this._shatter > 0) ? 'ice' : ((this.burnT > 0 || (this._charAcc || 0) > 0.8) ? 'fire' : (overkill ? 'gib' : 'flesh'));
    this.deathCause = cause;
    this.dying = this.dyingMax = (cause === 'ice' ? 0.06 : this.boss ? 1.2 : 0.62); this.dead = true;
    this.vx = (dir || sign(this.vx) || 1) * (this.boss ? 40 : (cause === 'fire' ? 90 : 160)); this.vy = -200;
    if (cause === 'ice') {
      // ESTÁTUA DE GELO QUEBRANDO: cacos gélidos + blocos congelados; nada de sangue
      this._noCorpse = true; this.vx = 0; this.vy = -60;
      game.fx.iceShatter(this.cx, this.cy, 26); game.fx.iceShatter(this.cx, this.y + this.h * 0.75, 14);
      game.fx.goreBurst(this.cx, this.cy, dir, 40 * big, '#bfe8ff');   // lascas que assentam no chão
      game.fx.goreChunks(this.cx, this.cy, dir, 10 * big, '#9fd4f2');  // blocos congelados maiores
      game.fx.frost(this.cx, this.cy, 6);
      game.cam.addShake(this.boss ? 12 : 4); Sound.crumble();
    } else if (cause === 'fire') {
      // MORTE EM CHAMAS: pira final + fumaça; o cadáver cai CARBONIZADO
      this._charAcc = Math.max(this._charAcc || 0, 2.2);
      game.fx.fire(this.cx, this.cy, 14 * big); game.fx.smoke(this.cx, this.cy, 9 * big);
      game.fx.spark(this.cx, this.cy, '#ff8a3c', 10);
      game.fx.goreBurst(this.cx, this.cy, dir, 24 * big, '#241812');   // bits chamuscados
      game.fx.scorch(this.cx, this.y + this.h - 2, 26);
      game.cam.addShake(this.boss ? 12 : 4); Sound.flesh();
    } else if (cause === 'gib') {
      // OVERKILL: o corpo se desfaz — explosão de gore total (estilo Broforce)
      this._noCorpse = true;
      game.fx.gib(this.cx, this.cy, gore, 34 * big);
      game.fx.blood(this.cx, this.cy, dir, 38 * big, gore);
      game.fx.smoke(this.cx, this.cy, 8 * big);
      game.fx.goreBurst(this.cx, this.cy, dir, 130 * big, gore);
      game.fx.goreChunks(this.cx, this.cy, dir, 30 * big, gore);
      game.cam.addShake(this.boss ? 16 : 7); Sound.flesh();
    } else {
      // morte padrão: sangue farto, MAS o corpo permanece — cadáver no chão
      game.fx.gib(this.cx, this.cy, gore, 18 * big);
      game.fx.blood(this.cx, this.cy, dir, 22 * big, gore);
      game.fx.smoke(this.cx, this.cy, 5 * big);
      game.fx.goreBurst(this.cx, this.cy, dir, 44 * big, gore);
      game.fx.goreChunks(this.cx, this.cy, dir, 8 * big, gore);
      game.fx.bloodPool(this.cx, this.y + this.h - 2, this.def.gore, 4);
  ...
  update(dt, game) {
    ...
    if (this.dying != null) {                 // corpo tombando: cai e vira CADÁVER persistente
      ...
      if (this.deathCause === 'fire' && Math.random() < 0.5) game.fx.fire(this.cx + rand(-6, 6), this.cy + rand(-8, 8), 1);   // ainda arde enquanto tomba
      if (this.dying <= 0) {
        this.alive = false;
        if (!this._noCorpse) game.fx.addCorpse(this);   // o corpo FICA no chão
      }
      return;
    }
    this.stagger = Math.max(0, this.stagger - dt);
    this._shatter = Math.max(0, (this._shatter || 0) - dt);
    this.attackT = Math.max(0, this.attackT - dt * 5);
    this.attackArc = Math.max(0, (this.attackArc || 0) - dt * 4);   // anim de golpe (braço varrendo)
    ...
    if (this.burnT > 0 || this.poisonT > 0) {
      ...
      // o CORPO pega fogo enquanto queima: as chamas brotam da PRÓPRIA silhueta
      // do frame atual (SPR.bodyPoint) — nada de caixas; fuligem acumula (_charAcc)
      if (this.burnT > 0) {
        this._charAcc = (this._charAcc || 0) + dt;
        if (Math.random() < 0.85) { const bp = (typeof SPR !== 'undefined' && SPR.bodyPoint) ? SPR.bodyPoint(this) : { x: this.cx, y: this.cy }; game.fx.fire(bp.x, bp.y, 1); }
        if (Math.random() < 0.12) game.fx.smoke(this.cx + rand(-4, 4), this.y + 2, 1);
      }
      // veneno: bolhas verdes sobem da pele e estouram
      if (this.poisonT > 0 && Math.random() < 0.4) { const bp = (typeof SPR !== 'undefined' && SPR.bodyPoint) ? SPR.bodyPoint(this) : { x: this.cx, y: this.cy }; game.fx.bubble(bp.x, bp.y); }
    }
    if (this.burnT <= 0) this._charAcc = Math.max(0, (this._charAcc || 0) - dt * 2);   // a fuligem esfria/clareia
    ...
    if (this.freezeT > 0 || this.sleepT > 0) {
      ...
      if (wasFrozen) {
        ...
        // névoa gelada escorrendo do corpo congelado
        if (Math.random() < 0.3) game.fx.frost(this.cx + rand(-this.w * 0.4, this.w * 0.4), this.y + rand(2, this.h * 0.8));
    ...
    if (target && this.touchCd <= 0 && aabb(this, target)) {
      this.attackT = 1; this.attackArc = 1; target.hurt(def.touch, sign(dx) || 1, game);
    ...
    if (this.touchCd <= 0 && game.summons && game.summons.length) {
      for (const s of game.summons) if (s.alive && aabb(this, s)) { this.attackT = 1; this.attackArc = 1; s.hurt(def.touch + 4, sign(s.cx - this.cx) || 1, game); this.touchCd = 0.7; this.vx -= sign(s.cx - this.cx) * 40; break; }
    }
    this.anim += dt;
  ...
  attack(game, target) {
    ...
    if (atk === 'blast') {
      ...
      game.fx.muzzle(m.x, m.y, this.aimAng); game.fx.shell(this.cx, this.y + this.h * 0.4, this.face); Sound.shot('shotgun');
    } else if (atk === 'smg') {
      ...
      game.fx.muzzle(m.x, m.y, this.aimAng); game.fx.shell(this.cx, this.y + this.h * 0.4, this.face); Sound.shot('mg');
    } else if (atk === 'rifle') {
      game.bullets.push(new Bullet(m.x, m.y, this.aimAng, 760, { faction: 'enemy', dmg: 14, kind: 'pellet', color: '#ffe27a', r: 3, tileDmg: 8 }));
      game.fx.muzzle(m.x, m.y, this.aimAng); game.fx.shell(this.cx, this.y + this.h * 0.4, this.face); game.cam.addShake(2); Sound.shot('rifle');
    } else if (atk === 'cannon') {
  ...
  _atkFenrir(game, target, m) {
    ...
    if (this.phase === 0) {
      ...
      this.attackT = 1; this.attackArc = 1; game.fx.shock(this.cx, this.y + this.h, 4 * CONFIG.TILE, '#caa33a'); game.cam.addShake(6);
  ...
  _atkTitan(game, target, m) {
    this.phase = (this.phase + 1) % 3;
    if (this.phase === 0) {
      this.attackT = 1; this.attackArc = 1; game.fx.shock(this.cx + this.face * 40, this.cy, 3 * CONFIG.TILE, '#8a929d');
  ...
  draw(ctx, cam) {
    ...
    // arma branca do chefe visível durante o golpe (ex.: cutelo do Carrasco)
    if (this.attackArc > 0 && this.dying == null && this.def.meleeVisual && typeof SPR !== 'undefined' && SPR.attackHand) {
      const hand = SPR.attackHand(this, 1 - clamp(this.attackArc, 0, 1));
      if (hand) SPR.drawMeleeSwing(ctx, hand.x + cam.ox, hand.y + cam.oy, hand.scale * 1.5, hand.ang, this.def.meleeVisual, this.def.meleeBlade || '#cfd6de', false);
    }
  ...
  // efeitos de status NO CORPO (sem retângulos): a silhueta do frame atual é
  // tintada (SPR.drawTinted) e adornos brotam do próprio corpo.
  _drawStatus(ctx, cam) {
    if (this.dying != null) return;
    const spr = typeof SPR !== 'undefined' && this.spr && SPR.defs[this.spr];
    const x = this.x + cam.ox, y = this.y + cam.oy, cx = x + this.w / 2;
    if (this.freezeT > 0) {
      // CONGELADO: casca de gelo vestindo o corpo + cristais brotando dele.
      // Tremor crescente perto do degelo (o inimigo luta p/ se libertar).
      const shiver = this.freezeT < 1.3 ? Math.sin(this.anim * 42) * 2.2 : 0;
      if (spr) {
        SPR.drawTinted(ctx, this, cam, '#9fd4f2', 0.62, shiver, 0);                   // gelo envolvendo o corpo
        SPR.drawTinted(ctx, this, cam, '#eaf8ff', 0.28, shiver - this.face * 1.5, -1.5); // brilho da casca (lado da luz)
      } else { ctx.fillStyle = 'rgba(150,210,255,0.35)'; ctx.fillRect(x - 1, y - 1, this.w + 2, this.h + 2); }
      // cristais determinísticos ancorados no corpo (ombros/costas/quadril)
      if (this._iceSeed == null) this._iceSeed = (Math.random() * 997) | 0;
      const sd = this._iceSeed, W = this.w, H = this.h;
      ctx.save(); ctx.translate(cx + shiver, y); ctx.fillStyle = 'rgba(234,248,255,0.92)';
      for (let i = 0; i < 5; i++) {
        const hx = Math.sin(sd + i * 2.4) * W * 0.34, hy = H * (0.12 + ((sd >> i) % 5) * 0.16);
        const sz = 3 + ((sd + i * 3) % 3), tilt = Math.sin(sd * 3 + i) * 0.5;
        ctx.beginPath();
        ctx.moveTo(hx - sz * 0.6, hy); ctx.lineTo(hx + sz * (0.5 + tilt * 0.4), hy - sz * 1.9); ctx.lineTo(hx + sz * 0.7, hy + sz * 0.3);
        ctx.closePath(); ctx.fill();
      }
      // glint que percorre a casca (bem sutil)
      const gt = (this.anim * 0.7) % 1;
      ctx.globalAlpha = 0.7 * Math.sin(gt * Math.PI);
      ctx.fillStyle = '#fff'; ctx.fillRect(-W * 0.34 + gt * W * 0.68, H * 0.2, 1.6, 5);
      ctx.restore();
    } else if (this.slowT > 0 && spr) {
      // LENTIDÃO: corpo levemente arroxeado pulsando devagar (sem caixa)
      SPR.drawTinted(ctx, this, cam, '#9b7bff', 0.15 + 0.06 * Math.sin(this.anim * 5));
    }
    if (this.burnT > 0) {
      // EM CHAMAS: o corpo acende com flicker de brasa (as línguas de fogo
      // reais sobem da silhueta — ver update); quanto mais queima, mais chamusca
      if (spr) {
        const fl = 0.24 + 0.13 * Math.sin(this.anim * 21) + 0.07 * Math.sin(this.anim * 47 + 2);
        SPR.drawTinted(ctx, this, cam, '#ff7a2c', fl);
        SPR.drawTinted(ctx, this, cam, '#ffd86b', fl * 0.5, 0, -1.5);                 // calor subindo (deslocado p/ cima)
        SPR.drawTinted(ctx, this, cam, '#241008', Math.min(0.30, (this._charAcc || 0) * 0.05)); // fuligem acumulando
      } else { ctx.save(); ctx.globalAlpha = 0.2 + 0.1 * Math.sin(this.anim * 18); ctx.fillStyle = '#ff7a2c'; ctx.fillRect(x - 1, y - 1, this.w + 2, this.h + 2); ctx.restore(); }
    }
    if (this.poisonT > 0 && spr) {
      // ENVENENADO: pele esverdeada pulsante (bolhas sobem do corpo — update)
      SPR.drawTinted(ctx, this, cam, '#8ef06a', 0.15 + 0.08 * Math.sin(this.anim * 9));
    }
